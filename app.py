...
df['screen_size'] = df['screen_size'].astype(str).apply(str_to_list)
df['memory'] = df['memory'].astype(str).apply(str_to_list)
df['battery_size'] = df['battery_size'].astype(str).apply(str_to_list)
df['phonprice_dist'] = df['phonprice_dist'].apply(str_to_list)
df['ram'] = df['ram'].astype(str).apply(str_to_list)
...
def recommend(phone):
    ...
    for i in phones_list:
        if df2.iloc[i[0]].phonename != "xyz":
            tups = (
                'Company : ' + df2.iloc[i[0]].phonecompany,
                'Model : ' + df2.iloc[i[0]].phonename,
                'Price : ' + str(df2.iloc[i[0]].phonprice)
            )
            l.append(tups)
    return l
...
if st.button('Recommend'):
    ...
    st.markdown('<p class="big-font">Here are the  <i>RECOMMENDATIONS</i>  for you ! &#128241</p>',
                unsafe_allow_html=True)
    st.write("--------------------------------------------------------------------------")
